...
async def _task_can_open_websocket(safe_task_id: str) -> bool:
    redis_async_client = await redis_manager.get_client()
    if await redis_async_client.exists(f"task_id:{safe_task_id}"):
        return True
    ...
    return (
        (MESSAGES_DIR / f"{safe_task_id}.json").exists()
        or (WORK_DIR_ROOT / safe_task_id).exists()
    )
...
async def websocket_endpoint(websocket: WebSocket, task_id: str):
    ...
    if not await _task_can_open_websocket(safe_task_id):
