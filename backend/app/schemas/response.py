...
class Message(BaseModel):
    ...
    结构化身份字段下沉到基类，保证 SystemMessage / ToolMessage /
    ProgressMessage 也能携带前端分组所需的 agent_instance_id、group_id、phase 等。
    这样普通系统卡片不再只能依赖中文内容解析。
    """
    ...
    # 结构化身份字段（所有消息统一可用）
    agent_instance_id: str | None = None  # 如 "q1.coder.main"、"q2.writer"
    question_index: int | None = None  # 所属问题编号（1-based）
    race_index: int | None = None  # 竞速 / 备用编号
    phase: str | None = None  # coordinating|modeling|coding|writing|reviewing|stopped
    group_id: str | None = None  # 前端分组用，如 "q1.coder.main"
