...
class E2BCodeInterpreter(BaseCodeInterpreter):
    ...
    async def get_created_images(self, section: str) -> list[str]:
        recorded_images = self.created_images_by_section.get(section, set())
        if recorded_images:
            moved = self.move_images_to_section_dir(list(recorded_images))
            self.save_section_code(section)
            return moved

        """获取当前 section 创建的图片列表，并归入对应子目录。"""
        if not self.sbx:
        ...
        try:
            ...
            section_images = set(self.section_output[section]["images"])
            new_images = list(section_images - set(self.created_images))
            self.created_images = list(section_images)
            logger.info(f"{section}-获取创建的图片列表: {new_images}")

            # 将新图片移动到对应章节目录
            moved = self.move_images_to_section_dir(new_images)
            self.save_section_code(section)
            return moved
        except Exception as e:
