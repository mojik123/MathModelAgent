...
import { ChevronDown, MessageCircle, RefreshCw, Send, Sparkles } from "lucide-vue-next";
...
const disabledBadgeText = computed(() => {
	if (props.regenerating) return "重新生成中";
	if (props.disabled) return "暂不可选";
	return "";
});
	...
	>
		...
		<div v-else class="flex flex-col">
			...
			>
				...
				<div class="min-w-0 flex-1">
					<div class="text-sm font-semibold text-slate-900">
						...
						<span v-if="disabledBadgeText" class="text-[10px] text-slate-400 ml-1">（{{ disabledBadgeText }}）</span>
					</div>
				...
				<button
					type="button"
					class="inline-flex shrink-0 items-center gap-1 rounded-md border border-blue-200 bg-white/75 px-2 py-1 text-[11px] font-medium text-blue-700 transition-colors hover:bg-blue-50 disabled:cursor-not-allowed disabled:opacity-55"
			...
			<div class="space-y-1.5 px-4 pt-3 max-h-64 overflow-y-auto">
				...
				<div
					v-if="regenerating"
					class="flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50/70 px-3 py-2 text-xs leading-relaxed text-blue-700"
				>
					<RefreshCw class="h-3.5 w-3.5 shrink-0 animate-spin" />
					<span>正在重新生成本问建模思路，当前选项暂时不可选。</span>
				</div>
				<div
