...
const regeneratingQuestionIndexes = ref<number[]>([]);
...
const hasQuestionRegenerating = computed(
	() => regeneratingQuestionIndexes.value.length > 0,
);

function isQuestionRegenerating(questionIndex: number) {
	return regeneratingQuestionIndexes.value.includes(questionIndex);
}

function setQuestionRegenerating(questionIndex: number, active: boolean) {
	const current = new Set(regeneratingQuestionIndexes.value);
	if (active) current.add(questionIndex);
	else current.delete(questionIndex);
	regeneratingQuestionIndexes.value = Array.from(current).sort((a, b) => a - b);
}
	...
	const questionGenStatus = computed<Record<number, { status: GenStatus; text: string }>>(() => {
		...
		for (let i = progressStartIndex; i < msgs.length; i++) {
			...
			if (c.includes("保底") || c.includes("降级")) result[idx] = { status: "done", text: "已生成保底候选，可继续修改" };
			if (c.includes("候选方案生成完成")) result[idx] = { status: "done", text: "生成完毕" };
		}
		// 已有 presetOptions 的标记为 done
		for (const q of questions.value) {
			if (isQuestionRegenerating(q.questionIndex)) {
				const current = result[q.questionIndex];
				if (!current || current.status === "waiting" || current.status === "done") {
					result[q.questionIndex] = {
						status: "generating",
						text: "正在重新生成候选模型...",
					};
				}
			} else if (q.presetOptions.length > 0 && result[q.questionIndex]?.status !== "done") {
...
async function handleRegenerateQuestion(questionIndex: number) {
	if (
		...
		return;
	const question = questions.value.find((q) => q.questionIndex === questionIndex);
	const questionSource = questionsList.value.find((q) => q.index === questionIndex);
	if (!question || !questionSource) return;

	const previousQuestion = { ...question };
	progressStartIndex = taskStore.messages.length;
	setQuestionRegenerating(questionIndex, true);
	optionsError.value = "";
	confirmError.value = "";
	...
	taskStore.addUserAction(
		"重生成",
		`第 ${questionIndex} 问建模思路`,
		`用户请求 ModelerAgent 单独重新生成第 ${questionIndex} 问的候选建模思路，并重新经过自动质检门禁。`,
		{
			from: "User",
			to: "ModelerAgent",
			label: "单问重新筛选",
	...
	try {
		...
		const generated = res.data.questions.find(
			(q) => q.questionIndex === questionIndex,
		);
		const options = normalizeModelOptions(generated?.options ?? []);
		if (!generated || options.length === 0) {
			throw new Error("后端没有返回可用候选方案");
		}
		handleQuestionsUpdate(
			questions.value.map((q) =>
				q.questionIndex === questionIndex
					? {
							...
							recommendedOptionId: generated.recommendedOptionId,
							selectedOptionId: "",
							customInput: "",
							confirmed: false,
						}
		...
		const recommended = options.find(
			(option) => option.id === generated.recommendedOptionId,
		);
		taskStore.addAgentAction(
			AgentType.MODELER,
			"重生成",
			`第 ${questionIndex} 问候选模型`,
			`ModelerAgent 已重新生成第 ${questionIndex} 问候选方案${
				recommended ? `，推荐：${recommended.label}` : ""
			}。`,
			{
				from: "ModelerAgent",
				to: "User",
		...
		const detail =
			...
			(error as { response?: { data?: { detail?: string } } }).response?.data
				?.detail;
		optionsError.value =
			detail || `第 ${questionIndex} 问重新生成失败，已保留原候选方案。`;
		handleQuestionsUpdate(
			questions.value.map((q) =>
				q.questionIndex === questionIndex ? previousQuestion : q,
			),
		);
	} finally {
		setQuestionRegenerating(questionIndex, false);
	}
}
...
async function handleConfirm() {
	...
	confirmError.value = "";
	if (hasQuestionRegenerating.value) {
		confirmError.value = `第 ${regeneratingQuestionIndexes.value.join("、")} 问正在重新生成建模思路，请完成后再确认。`;
		return;
	}
	...
	>
		...
		<div class="flex-1 min-h-0 overflow-y-auto px-4 py-4">
			<ModelingCardStack
				...
				:regenerating-question-indexes="regeneratingQuestionIndexes"
