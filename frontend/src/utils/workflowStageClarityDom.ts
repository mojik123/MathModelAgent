...
function questionWriterStarted(messages: Array<{ content?: string | null }>, index: number) {
	return messages.some((message) => {
		const text = messageText(message);
		return (
			new RegExp(`(?:\\[组#${index}\\]|子问题组#${index}).*论文手开始写`, "i").test(text) ||
...
function updateWritingStep() {
	...
	const activeQuestionWriters = Array.from({ length: 12 }, (_, i) => i + 1).filter(
		(index) => questionWriterStarted(messages, index),
	);
	...
	for (const node of document.querySelectorAll<HTMLElement>(
		...
		if (textNode) textNode.textContent = globalStarted ? " 论文写作" : " 分问章节写作";
		node.title = globalStarted
	...
	for (const node of document.querySelectorAll<HTMLElement>(
		...
		node.classList.remove("border-violet-200", "bg-violet-50", "text-violet-700");
